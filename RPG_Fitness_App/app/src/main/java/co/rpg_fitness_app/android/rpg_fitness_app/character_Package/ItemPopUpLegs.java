...
public class ItemPopUpLegs extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // retrieve all legss from the DB
        legsList = new ArrayList<Gear>();
        for (int i = 0; i < gearList.size(); i++) {
            if (gearList.get(i).getCategory().equals("Legs") && gearList.get(i).isOwned() ==  true) {
                if(gearList.get(i).isEquipped() == true) {
                    Log.d("index 0", "equipped legs : " + gearList.get(i).getName());
                    indexOfEquipped = i;
                    legsList.add(0,gearList.get(i));
                }
                else {
                    Log.d("index not 0", "not equipped legs : " + gearList.get(i).getName());
                    legsList.add(gearList.get(i));
                }
            }
        }
        ...
        if(character.getActiveGear().get(0) == null) {
            indexOfEquipped = 0;
            character.equipItem(legsList.get(0));
        ...
        for (int i = 0; i < legsList.size(); i++) {
            ...
            button.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    character.equipItem(legsList.get(index));
                    mDataSource.updateGear(legsList.get(indexOfEquipped));
                    mDataSource.updateGear(legsList.get(index));
            ...
            if(i == 0)
            {
                Log.d("change button text", "set text " + legsList.get(index).getName());
                button.setText(legsList.get(index).getName() + " (Eq.)");
            }
            else
            {
                button.setText(legsList.get(i).getName());
            }
