...
public class ItemPopUpOffHand extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // retrieve all offHands from the DB
        offHandList = new ArrayList<Gear>();
        for (int i = 0; i < gearList.size(); i++) {
            if (gearList.get(i).getCategory().equals("OffHand") && gearList.get(i).isOwned() ==  true) {
                if(gearList.get(i).isEquipped() == true) {
                    Log.d("index 0", "equipped offHand : " + gearList.get(i).getName());
                    indexOfEquipped = i;
                    offHandList.add(0,gearList.get(i));
                }
                else {
                    Log.d("index not 0", "not equipped offHand : " + gearList.get(i).getName());
                    offHandList.add(gearList.get(i));
                }
            }
        }
        ...
        if(character.getActiveGear().get(0) == null) {
            indexOfEquipped = 0;
            character.equipItem(offHandList.get(0));
        ...
        for (int i = 0; i < offHandList.size(); i++) {
            ...
            button.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    character.equipItem(offHandList.get(index));
                    mDataSource.updateGear(offHandList.get(indexOfEquipped));
                    mDataSource.updateGear(offHandList.get(index));
            ...
            if(i == 0)
            {
                Log.d("change button text", "set text " + offHandList.get(index).getName());
                button.setText(offHandList.get(index).getName() + " (Eq.)");
            }
            else
            {
                button.setText(offHandList.get(i).getName());
            }
